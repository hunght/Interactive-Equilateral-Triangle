...
function calculateHeightB(x, y) {
  const intersectBx = (y - TRIANGLE_HEIGHT + Math.sqrt(3) * x) / Math.sqrt(3);
  const intersectBy = TRIANGLE_HEIGHT - Math.sqrt(3) * (intersectBx - 1);
  return {
    intersectBx,
    intersectBy,
    length: Math.sqrt(
...
describe('Height B calculations', () => {
  ...
  test('Height B at bottom-right vertex', () => {
    ...
    expect(result.intersectBx).toBeCloseTo(1, 5);
    expect(result.intersectBy).toBeCloseTo(0, 5);
    expect(result.length).toBeCloseTo(0, 5);
  });

  test('Height B at a random point', () => {
    const result = calculateHeightB(0.7, 0.4);
    expect(result.intersectBx).toBeCloseTo(0.9619, 4);
    expect(result.intersectBy).toBeCloseTo(0.2887, 4);
    expect(result.length).toBeCloseTo(0.2722, 4);
  });
});
